...
class ReversibleDMP():
    """
        The DMP class that implements discrete reversible DMP
    """
    ...
    def train(self):
        ...
        x[0,0] = 1.
        t = 0
        # ----- Euler solution to exponentially decreasing canonical system
    ...
    def generate_forward_trajectory(self, config=None):
        """
            this function is used to generate a new (forward) DMP trajectory from already trained system
            Args: 
        ...
        # ----- generate the trajectory by rolling out one point at a time
        while u > 1e-3:
    ...
    def generate_reverse_trajectory(self, config = None):
        """
            this function generates a new DMP trajectory from the already trained system, and takes its reverse
            Args: 
            config : contains all parameters like the input
                dt: time step
                K: porportional gain of the dmp
        ...
        # ----- the reversed trajectory will have the goal and start opposite to that of the actual trajectory
        forward_config['goals'] = config['reversed_start']
        forward_config['y0'] = config['reversed_goal']

        forward_traj = self.generate_forward_trajectory(config = forward_config)


        reverse_traj = {
        # -----computed traj of the trajectory
        'pos':np.flipud(forward_traj['pos']),
        # -----computed vel of the trajectory
        'vel':np.flipud(forward_traj['vel']),
        # -----computed acc of the trajectory
        'acc':np.flipud(forward_traj['acc']),
        }
